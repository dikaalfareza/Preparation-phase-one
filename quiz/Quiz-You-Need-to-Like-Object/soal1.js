...
function ganjilGenapAsianGames(date, data) {
  ...
  for (let i = 0; i < data.length; i++) {
    if (data[i].type === "Motor") continue;
    let plat = data[i].plat.split(" ")[1];
    platMobil.push(plat[plat.length - 1]);
  }

  let isGenap = date % 2 === 0;
  let counter = 0;
  if (!isGenap) {
    for (let j = 0; j < platMobil.length; j++) {
      if (platMobil[j] % 2 === 0) counter++;
    }
  } else {
    for (let j = 0; j < platMobil.length; j++) {
      if (platMobil[j] % 2 !== 0) counter++;
