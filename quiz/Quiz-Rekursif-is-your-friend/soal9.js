...
function virusCheckRecursive(str, viruses) {
  //code here
  if (!str || !viruses) return "There is no virus";

  let virus = viruses.length > 1 ? viruses.split("|") : viruses;
  let char = str[0].toLowerCase();
  let isVirus = false;

  for (let i = 0; i < virus.length; i++) {
    if (char === virus[i].toLowerCase()) isVirus = true;
  }

  if (str.length === 1) return isVirus ? 1 : 0;
  return isVirus
    ? 1 + virusCheckRecursive(str.slice(1), viruses)
    : virusCheckRecursive(str.slice(1), viruses);
}
